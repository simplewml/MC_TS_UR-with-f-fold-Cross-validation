function [CEtrain,CEvalidation,CEtest,BT,CT,SigmaT,VT]=MC(XTr,yTr,XVa,yVa,XTe,yTe,alpha,rr,P,numMFs,numIt,plabel,batchSize,PCA_dims)
...
for it=1:numIt 
    ...
    for n=1:batchSize%??????????
        ...
        for r=1:R%??????
            temp=-sum((yTrain(idsTrain(n),:)./ycPred(n,:)-(1.-yTrain(idsTrain(n),:))./(1.-ycPred(n,:))).*ycPred(n,:).*(1.-ycPred(n,:)).*V(r,:).*(yR(r)*sum(f(n,:))-f(n,:)*yR')/sum(f(n,:))^2*f(n,r));
