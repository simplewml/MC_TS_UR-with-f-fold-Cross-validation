function [CEtrain,CEvalidation,CEtest,BT,CT,SigmaT,VT]=MC_TS_UR(XTr,yTr,XVa,yVa,XTe,yTe,alpha,rr,lam,P,numMFs,numIt,plabel,batchSize,gama,PCA_dims)
...
%     deltaV=rr*V;
    ...
    for n=1:batchSize%??????????
        ...
        for r=1:R%??????
            temp=-sum((mnyTrain(n,:)./ycPred(n,:)-(1.-mnyTrain(n,:))./(1.-ycPred(n,:))).*ycPred(n,:).*(1.-ycPred(n,:)).*V(r,:).*(yR(r)*sum(f(n,:))-f(n,:)*yR')/sum(f(n,:))^2*f(n,r))-2*lam*(fBar(r)-1/R)*fBar(r)*(1-fBar(r));%?n????????*???r???-yPred)*??r????????
